...
const cwd = exampleDir("netlify-express");

describe("netlify-express", () => {
	describe("dev", () => {
		...
		it("renders / via Express (DEV branch)", async () => {
		...
		it("renders dynamic route via Express splat", async () => {
	...
	describe("preview", () => {
		...
		it("serves dynamic route via Express splat", async () => {
			const res = await fetch(`${server.url}/anything`);
			assert.equal(res.status, 200);
			assert.match(await res.text(), /^Running \/anything in /);
		});
