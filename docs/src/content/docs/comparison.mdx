...
**[Nitro](https://nitro.build/) is a server framework.** It's an opinionated toolkit that turns your Vite project into a production server with its own routing, key-value storage, caching, and database conventions. It can deploy that server to Node.js, Cloudflare Workers, Deno, Bun, AWS Lambda, Vercel, Netlify, and more.

Key differences:

- Nitro is opinionated: it owns routing, storage, and caching. Vite Deploy is the opposite: it deploys whatever `fetch` handler you give it and stays out of the rest.
- Nitro decides what your server entry looks like; Vite Deploy lets you write your own.

**Use Nitro if you want a batteries-included server framework.** Use Vite Deploy if you already have a handler (or a framework) and just want a deployment layer.
